...
if(apiPost('submit') && !$likelySpam) {
	...
	$message = "<p>Hi $firstName!<br><br>\n"
		...
		. "If you have paid your \$" . (getAge($dateOfBirth) >= $primaryCastMinimumAge ? $campRegistrationFee : $campRegistrationFeeSecondary) . " registration fee then you are guaranteed a spot in the show!<br><br>\n"
	...
	header("Location: pay.php?amount=" . (getAge($dateOfBirth) >= $primaryCastMinimumAge ? $campRegistrationFee : $campRegistrationFeeSecondary) . "&name={$parentName}&email={$parentEmail}&phone={$parentPhone}&comments={$currentShow->getAbbrUcFirst()}%20Registration%20Fee:%20{$firstName}%20{$lastName}");
}
		...
		<form action='<?=$_SERVER['PHP_SELF'];?>' method='post' role='form' class='bs-condensed'>
			...
			<div class='row'>
				...
				<section class='col-sm-3'>
					<select name='chosenWeek' class='form-control' required>
						<option value='' selected>Select One...</option>
						<option value='1'>Week 1 (March 14-19, 2016)</option>
						<option value='2'>Week 2 (March 21-26, 2016)</option>
					</select>
				</section>
			</div>
			<div class='row'>
			...
			<div class='row'>
				<label class='col-sm-3 label-control text-right'>
					Please list any conflicts with the rehearsal schedule and/or performances (Week 1: March 14-19 or Week 2: March 21-26):
				</label>
				<section class='col-sm-9 col-lg-6'>
			...
			<div class='row'>
				<section class='col-sm-12 text-center top-buffer'>
					<input type='submit' name='submit' value='Submit Form And Pay Registration Fee' onclick='return confirm("Is the form complete and accurate?")' class='btn btn-primary'>
				</section>
			</div>
		</form>
	</section>
